...
export async function handleGetTeamWorkloadSummary(args) {
  ...
  const results = await Promise.all(input.accountIds.map((accountId) => getUserHours(accountId, input.from, input.to)));
  return { content: [{ type: 'text', text: JSON.stringify(summarizeWorkload(results, input), null, 2) }] };
}
